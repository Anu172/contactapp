...
server.get("/showcontact", (req, res) => {
  ...
  res.status(status).json(userdb);
});

server.post("/savecontact", (req, res) => {
  const { email, name, phone, emailCheck } = req.body;
  ...
  fs.readFile("./users.json", (err, data) => {
    ...
    data = JSON.parse(data.toString());
    let last_item_id = data.contacts[data.contacts.length - 1].id;
    data.contacts.push({
      id: last_item_id + 1,
      name: name,
      email: email,
